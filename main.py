...
from src import data_handling, temporal_clustering, model_runner, postprocessing
...
def main():
    ...
    # 1. Load the PyPSA Europe test case
    logging.info(f"Loading network from {config['pypsa_eur_test_case_path']}")
    # n_full = data_handling.load_network(config['pypsa_eur_test_case_path'])
    # For now, let's create a dummy network to avoid a FileNotFoundError
    n_full = pypsa.Network()
    n_full.add("Bus", "MyBus")
    n_full.add("Load", "MyLoad", bus="MyBus", p_set=[10, 20, 30])
    n_full.set_snapshots(range(3))
    logging.info("Dummy network created.")


    # 2. Cluster the test case temporally
