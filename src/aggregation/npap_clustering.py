...
def aggregate(n: pypsa.Network, config: dict, label: str) -> pypsa.Network:
    ...
    if 's_nom_extendable' in n.lines.columns:
        ...
        logging.info(f"Mapped 's_nom_extendable' to {extendable_any.sum()} aggregated lines based on original line data.")
    ...
    if 's_nom_extendable' in n.transformers.columns and not network.transformers.empty:
        ...
        if not valid_traftomap.empty:
            ...
            logging.info(f"Mapped 's_nom_extendable' to {extendable_any_trafo.sum()} aggregated transformers based on original transformer data.")
